...
- `hsbi_claim_rewards` now imports `KeysDB`, consumes the `AccountsDB` voting list, and falls back to runtime accounts so it always claims for the configured operators (`hsbi_claim_rewards.py:24-40`).
...
- Restricting `PostsTrx.get_unvoted_post` to `main_post=1` still removes every comment from the queue, so the existing comment-specific branches in `hsbi_upvote_post_comment.py` will never trigger even though they are still in the codepath (`hivesbi/transfer_ops_storage.py:372-382`, `hsbi_upvote_post_comment.py:237-360`).
