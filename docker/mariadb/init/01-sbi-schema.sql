...
CREATE TABLE `token_issuance_log` (
  ...
  `last_resolution_attempt` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_recipient` (`recipient`),
  KEY `idx_trx_id` (`trx_id`),
  KEY `idx_source_trx_id` (`source_trx_id`),
  KEY `idx_status_attempt` (`status`,`last_resolution_attempt`,`issued_at`)
...
CREATE TABLE `tokenholders` (
  ...
  `virtual_tokens` decimal(15,3) NOT NULL DEFAULT 0.000,
  `tokens` decimal(15,3) GENERATED ALWAYS AS (`liquid_tokens` + `LP_tokens` + `virtual_tokens`) VIRTUAL,
